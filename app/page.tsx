...
type EntryRow = {
  ...
  waku: number | null;
  umaban: number | null;
  horses?: { horse_name: string | null }[] | null;
};
...
export default function Home() {
  ...
  async function loadRaceView(raceId: string) {
    ...
    const { data: entries, error: eErr } = await supabase
      .from('entries')
      .select('race_id,horse_id,waku,umaban,horses(horse_name)')
      .eq('race_id', raceId)
    ...
    // ★ここ：race_name/surface/distance_m も取る（列があっても無くても動くように select('*')）
    ...
    const view: HorseViewRow[] = ent.map((x) => ({
      ...
      horse_name: (x.horses?.[0]?.horse_name ?? '(不明)') as string,
